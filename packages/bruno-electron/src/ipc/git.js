...
const { cloneGitRepository, getCollectionGitRootPath } = require('../utils/git');
const { createDirectory, removePath, exists } = require('../utils/filesystem');
...
const getCollectionGitContext = async (collectionPath) => {
  ...
  const [currentGitBranch, gitRepoUrl, changedFilesRaw] = await Promise.all([
    git.revparse(['--abbrev-ref', 'HEAD']).then((branch) => branch.trim()).catch(() => ''),
